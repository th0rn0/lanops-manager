...
generate-key-show-newkey:
	docker run --rm composer:2.6 /bin/bash -c "echo 'generating key..' && composer create-project laravel/laravel example-app >/dev/null 2>/dev/null && cd example-app && php artisan key:generate >/dev/null 2>/dev/null && cat .env | grep APP_KEY=b"
...
composer-install:
    ...
    --user 82:82 \
    composer:2.6 install --ignore-platform-reqs --no-scripts
...
composer-install-dev:
    ...
    -v $(currentDir)/src:/app \
    $(user) \
    composer:2.6 install --ignore-platform-reqs --no-scripts --dev
...
composer-update:
    ...
    --volume $(currentDir)/src:/app \
    $(user) \
    composer:2.6 update --ignore-platform-reqs --no-scripts

# Suggest Dev PHP Dependencies via Composer
composer-suggest:
	docker run --rm --name compose-maintainence-update --interactive \
    --volume $(currentDir)/src:/app \
    $(user) \
    composer:2.6 suggest
...
composer-add-dep:
    ...
    --volume $(currentDir)/src:/app \
    $(user) \
    composer:2.6 require $(module) --ignore-platform-reqs --no-scripts
...
composer-add-dep-dev:
    ...
    --volume $(currentDir)/src:/app \
    $(user) \
    composer:2.6 require $(module) --ignore-platform-reqs --no-scripts --dev
