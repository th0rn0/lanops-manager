				...
				{{ Form::open(array('url'=>'/admin/events/' . $event->slug . '/tournaments/' . $tournament->slug )) }}
					...
					@if ($tournament->status != 'LIVE' && $tournament->status != 'COMPLETE')
						...
						<div class="form-group">
							<div class="checkbox">
								<label>
									<input type="checkbox" class="form-control"  name="only_signedin"  @if(old('only_signedin', $tournament->only_signedin)) checked @endif> Signup only when signed in to event
								</label>
							</div>
						</div>
					@endif
